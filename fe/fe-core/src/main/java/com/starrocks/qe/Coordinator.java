...
public class Coordinator {
    ...
    public Coordinator(InsertPlannerV2 insertPlanner) {
        // LOG.warn("new planner");
        this.etlJobType = insertPlanner.getEtlJobType();
        if (etlJobType == EtlJobType.INSERT) {
            this.isBlockQuery = false;
        } else {
            this.isBlockQuery = true;
        }
        this.jobId = insertPlanner.getLoadJobId();
        ConnectContext context = insertPlanner.getContext(); 
        this.queryId = insertPlanner.getLoadId();
        this.connectContext = context;
        this.descTable = insertPlanner.getDescTable().toThrift();
        this.fragments = insertPlanner.getFragments();
        this.scanNodes = insertPlanner.getScanNodes();

        this.queryOptions = context.getSessionVariable().toThrift();
        this.queryOptions.setQuery_type(TQueryType.LOAD);
        this.queryOptions.setQuery_timeout((int) insertPlanner.getTimeout());
        this.queryOptions.setMem_limit(insertPlanner.getExecMemLimit());
        this.queryOptions.setLoad_mem_limit(insertPlanner.getLoadMemLimit());
        Map<String, String> sessionVariables = insertPlanner.getSessionVariables();
        ...
        long startTime = insertPlanner.getStartTime();
        String timezone = insertPlanner.getTimeZone();
